...
class MemorySoure : public FramedSource
{
public:

	MemorySoure(UsageEnvironment& env,
		char const* streamName):FramedSource(env){
			unsigned threadid = 0; 
			_hFile = fopen("test.264","rb");
		//	_hThread = (HANDLE)_beginthreadex(NULL,0,_threadFun,this,0,&threadid);
	}

	static MemorySoure* createNew(UsageEnvironment& env, char const* streamName){
		MemorySoure* newSource = new MemorySoure(env,streamName);
		return newSource;

	}

	static unsigned int WINAPI _threadFun(void* pParam){
		MemorySoure* pThis = reinterpret_cast<MemorySoure*>(pParam);
		return pThis->doRun();
	}
	...
	unsigned int doRun(){
		_hFile = fopen("test.264","rb");
		if(!_hFile){
			*env<<"open test.264 file error...\r\n";
			return -1;
		}
		char* buf[4096] = {0};
		while(True){			
			int iSize = fread(buf,1,4096,_hFile);
			if (iSize > 0){
...
private:
	virtual void doGetNextFrame(){
		//fMaxSize = 4096;
		*env<<fMaxSize;
		fFrameSize = fread(fTo, 1, fMaxSize, _hFile);
		if (feof(_hFile)){
			fseek(_hFile,0,SEEK_SET);
		}	
...
class H264MemoryMediaServerSubsession :public H264VideoFileServerMediaSubsession
{
public:
	static H264MemoryMediaServerSubsession* createNew(UsageEnvironment& env,
		char const* streamName) {
			return new H264MemoryMediaServerSubsession(env, streamName);
	}
	H264MemoryMediaServerSubsession(UsageEnvironment& env,
		char const* streamName):H264VideoFileServerMediaSubsession(env,
		streamName,  True){
...
protected:
	virtual FramedSource* createNewStreamSource(unsigned clientSessionId,
		unsigned& estBitrate){
			...
			MemorySoure* memSource = MemorySoure::createNew(envir(), fFileName);
...
int main(int argc, char** argv)
{
...

	{
		...
		ServerMediaSession* sms
			...
			descriptionString);
		sms->addSubsession(H264MemoryMediaServerSubsession
