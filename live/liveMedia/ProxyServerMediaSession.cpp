...
void ProxyServerMediaSession::reset()
{
	char url[1024] = {0};
	strcpy(url,fProxyRTSPClient->fOurURL);
	if (fProxyRTSPClient != NULL) 
		fProxyRTSPClient->sendTeardownCommand(*fClientMediaSession, NULL, fProxyRTSPClient->auth());
	/*Medium::close(fClientMediaSession);*/
	Medium::close(fProxyRTSPClient);
	fProxyRTSPClient
		= (*fCreateNewProxyRTSPClientFunc)(*this, url, NULL, NULL,
		0,
		fVerbosityLevel > 0 ? fVerbosityLevel-1 : fVerbosityLevel,
		-1);
	ProxyRTSPClient::sendDESCRIBE(fProxyRTSPClient);
...
void ProxyRTSPClient::proxyReset(void* clientData){
...
FramedSource* ProxyServerMediaSubsession::createNewStreamSource(unsigned clientSessionId, unsigned& estBitrate) {
  ...
  if (fClientMediaSubsession.readSource() == NULL) {
	...
	if(!strcmp(fClientMediaSubsession.mediumName(),"video"))
	increaseReceiveBufferTo(envir(),fClientMediaSubsession.rtpSource()->RTPgs()->socketNum(),2000000);
    if (verbosityLevel() > 0) {
  ...
  if (fHaveSetupStream) {
    ...
    if (proxyRTSPClient->fLastCommandWasPLAY) { // so that we send only one "PAUSE"; not one for each subsession
	  ...
	  proxyRTSPClient->sendTeardownCommand(fClientMediaSubsession.parentSession(), NULL, proxyRTSPClient->auth());
		//proxyRTSPClient->continueAfterLivenessCommand(-1,False);
